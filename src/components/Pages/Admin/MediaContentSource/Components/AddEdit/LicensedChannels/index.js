...
const { LicensedChannels } = featureConstants
...
const AddRSSFeed = ({
  ...
  const isEdit = useMemo(() => path === routeByName[LicensedChannels].edit, [
    path
  ])

  const translate = useMemo(
    () => ({
      title: isEdit ? t('Edit Channel') : t('Add Chanel'),
      name: t('Channel name'),
      category: t('Category'),
      logo: t('Logo'),
      url: t('URL'),
  ...
  useEffect(() => {
    if (!isEmpty(item) && item.feature.name === LicensedChannels && isEdit) {
      const {
        name,
  ...
  const categories = useCategoryOptions(LicensedChannels)
  ...
  return (
    <SideModal
      ...
      closeLink={routeByName[LicensedChannels].root}
