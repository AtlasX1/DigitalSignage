...
const { MediaRSS } = featureConstants
...
const AddRSSFeed = ({
  ...
  const isEdit = useMemo(() => path === routeByName[MediaRSS].edit, [path])
  ...
  useEffect(() => {
    if (!isEmpty(item) && item.feature.name === MediaRSS && isEdit) {
      const {
        name,
  ...
  const categories = useCategoryOptions(MediaRSS)
  ...
  return (
    <SideModal
      ...
      closeLink={routeByName[MediaRSS].root}
