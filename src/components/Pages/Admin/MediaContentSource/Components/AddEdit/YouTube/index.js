...
const { YouTube } = featureConstants
...
const AddRSSFeed = ({
  ...
  const isEdit = useMemo(() => path === routeByName[YouTube].edit, [path])
  ...
  useEffect(() => {
    if (!isEmpty(item) && item.feature.name === YouTube && isEdit) {
      const {
        name,
  ...
  const categories = useCategoryOptions(YouTube)
  ...
  return (
    <SideModal
      ...
      closeLink={routeByName[YouTube].root}
