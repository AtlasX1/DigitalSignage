...
const AddRSSFeed = ({
  ...
  const isEdit = useMemo(() => path === routeByName[Radio].edit, [path])
  ...
  useEffect(() => {
    if (!isEmpty(item) && item.feature.name === Radio && isEdit) {
      const {
        name,
  ...
  const categories = useCategoryOptions(Radio)
  ...
  return (
    <SideModal
      ...
      closeLink={routeByName[Radio].root}
